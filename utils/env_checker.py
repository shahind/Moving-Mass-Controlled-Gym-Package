...
def _check_nan(env: mmcgym.Env, check_inf: bool = True) -> None:
...
def _check_returned_values(
    env: mmcgym.Env, observation_space: spaces.Space, action_space: spaces.Space
) -> None:
    """
    Check the returned values by the env when calling `.reset()` or `.step()` methods.
    """
    # because env inherits from mmcgym.Env, we assume that `reset()` and `step()` methods exists
    obs = env.reset()
    ...
    if isinstance(env, mmcgym.GoalEnv):
...
def _check_spaces(env: mmcgym.Env) -> None:
    """
    Check that the observation and action spaces are defined
    and inherit from mmcgym.spaces.Space.
    """
    # Helper to link to the code, because mmcgym has no proper documentation
    mmcgym_spaces = " cf https://github.com/openai/mmcgym/blob/master/mmcgym/spaces/"

    assert hasattr(env, "observation_space"), (
        "You must specify an observation space (cf mmcgym.spaces)" + mmcgym_spaces
    )
    assert hasattr(env, "action_space"), (
        "You must specify an action space (cf mmcgym.spaces)" + mmcgym_spaces
    )

    assert isinstance(env.observation_space, spaces.Space), (
        "The observation space must inherit from mmcgym.spaces" + mmcgym_spaces
    )
    assert isinstance(env.action_space, spaces.Space), (
        "The action space must inherit from mmcgym.spaces" + mmcgym_spaces
    )


# Check render cannot be covered by CI
def _check_render(
    env: mmcgym.Env, warn: bool = True, headless: bool = False
...
def check_env(env: mmcgym.Env, warn: bool = True, skip_render_check: bool = True) -> None:
    """
    Check that an environment follows mmcgym API.
    This is particularly useful when using a custom environment.
    Please take a look at https://github.com/openai/mmcgym/blob/master/mmcgym/core.py
    for more information about the API.
    It also optionally check that the environment is compatible with Stable-Baselines.
    :param env: The mmcgym environment that will be checked
    :param warn: Whether to output additional warnings
    ...
    assert isinstance(
        env, mmcgym.Env
    ), "Your environment must inherit from the mmcgym.Env class cf https://github.com/openai/mmcgym/blob/master/mmcgym/core.py"
