...
from mmcgym.spaces import Space, Box, Discrete, MultiDiscrete, MultiBinary, Tuple, Dict

_BasemmcgymSpaces = (Box, Discrete, MultiDiscrete, MultiBinary)
__all__ = ["_BasemmcgymSpaces", "batch_space"]
...
def batch_space(space, n=1):
    ...
    Parameters
    ----------
    space : `mmcgym.spaces.Space` instance
    ...
    Returns
    -------
    batched_space : `mmcgym.spaces.Space` instance
        Space (e.g. the observation space) for a batch of environments in the
        vectorized environment.

    Example
    -------
    >>> from mmcgym.spaces import Box, Dict
    >>> space = Dict({
    ...
    """
    if isinstance(space, _BasemmcgymSpaces):
        ...
        raise ValueError(
            ...
            "be a valid `mmcgym.Space` instance.".format(type(space))
        )
