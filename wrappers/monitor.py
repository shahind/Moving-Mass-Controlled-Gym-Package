...
from mmcgym import Wrapper
from mmcgym import error, version, logger
from mmcgym.wrappers.monitoring import stats_recorder, video_recorder
from mmcgym.utils import atomic_write, closer
from mmcgym.utils.json_utils import json_encode_np

FILE_PREFIX = "openaimmcgym"
...
class Monitor(Wrapper):
    def __init__(
        ...
        logger.deprecation(
            "The Monitor wrapper is being deprecated in favor of mmcgym.wrappers.RecordVideo and mmcgym.wrappers.RecordEpisodeStatistics (see https://github.com/openai/mmcgym/issues/2297)"
        )
    ...
    def _start(
        ...
        Args:
            ...
            force (bool): Clear out existing training data from this directory (by deleting every file prefixed with "openaimmcgym.").
        ...
        if self.env.spec is None:
            logger.warn(
                "Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'mmcgym.make', and is recommended only for advanced users."
            )
            env_id = "(unknown)"
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.""".format(
        ...
        self.enabled = True
        # We use the 'openai-mmcgym' prefix to determine if a file is
        # ours
    ...
    def close(self):
        ...
        logger.info(
            """Finished writing results. You can upload them to the scoreboard via mmcgym.upload(%r)""",
            self.directory,
        )
    ...
    def _env_info(self):
        env_info = {
            "mmcgym_version": version.VERSION,
        }
        if self.env.spec:
...
# not intended for use outside of the mmcgym codebase.
def _open_monitors():
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for key in ["env_id", "mmcgym_version"]:
        if key not in first:
            raise error.Error(
                "env_info {} from training directory {} is missing expected key {}. This is unexpected and likely indicates a bug in mmcgym.".format(
