...
from mmcgym import error, logger
...
# Whitelist of plugins which can hook into the `mmcgym.envs.internal` entry point.
plugin_internal_whitelist = {"ale_py.mmcgym"}
    ...
    def make(self, **kwargs):
        ...
        if env.spec.max_episode_steps is not None:
            ...
            if self.order_enforce:
                from mmcgym.wrappers.order_enforcing import OrderEnforcing
    ...
    def spec(self, path):
        if ":" in path:
            ...
            try:
                ...
                raise error.Error(
                    "A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `mmcgym.make()`".format(
                        mod_name
                    )
                )
        else:
        ...
        try:
            ...
            if matching_envs:
                ...
                raise error.UnregisteredEnv(
                    "Algorithmic environment {} has been moved out of mmcgym. Install it via `pip install mmcgym-algorithmic` and add `import mmcgym_algorithmic` before using it.".format(
                        id
                    )
                )
            elif env_name in toytext_envs:
                raise error.UnregisteredEnv(
                    "Toytext environment {} has been moved out of mmcgym. Install it via `pip install mmcgym-legacy-toytext` and add `import mmcgym_toytext` before using it.".format(
                        id
                    )
                )
            else:
...
def load_env_plugins(entry_point="mmcgym.envs"):
    ...
    for plugin in metadata.entry_points().get(entry_point, []):
        ...
        try:
            ...
            if attr is None:
                raise error.Error(
                    f"mmcgym environment plugin `{module}` must specify a function to execute, not a root module"
                )
